...
// Get students for parent of certain class.
function get_students_for_parent($class_id = '') {
  $current_user = get_currentuserinfo();
  $current_user_name = $current_user->user_login;
  // echo $current_user_name.'<br/>';
  global $wpdb;
  $student_id;
  $parent_table = $wpdb->prefix . 'ekattor_parent';
  $student_id_array = $wpdb->get_results("SELECT student_id FROM $parent_table WHERE username = '$current_user_name' ", ARRAY_A);
  foreach ($student_id_array as $key) {
    $student_id = $key['student_id'];
  }
  //echo $student_id.'<br/>';
  $school_student = $wpdb->prefix . 'ekattor_student';
  $students = $wpdb->get_results("SELECT * FROM $school_student WHERE student_id = '$student_id' AND class_id = '$class_id'", ARRAY_A);
  //print_r($students);
  return $students;
}
...
// Get all classes.
function get_classes_for_parents_child() {
  $current_user = get_currentuserinfo();
  $current_user_name = $current_user->user_login;
  ...
  $class_id;
  $parent_table = $wpdb->prefix . 'ekattor_parent';
  $student_id_array = $wpdb->get_results("SELECT * FROM $parent_table WHERE username = '$current_user_name' ", ARRAY_A);
  foreach ($student_id_array as $key) {
    $student_id = $key['student_id'];
  }
  //echo $student_id.'<br/>';
  $student_table = $wpdb->prefix . 'ekattor_student';
  $student_details_array = $wpdb->get_results("SELECT * FROM $student_table WHERE student_id = '$student_id' ", ARRAY_A);
  foreach ($student_details_array as $key) {
    $class_id = $key['class_id'];
  }
  $student_class = $wpdb->prefix . 'ekattor_class';
  $class = $wpdb->get_results("SELECT * FROM $student_class WHERE class_id = '$class_id'", ARRAY_A);
  //print_r($students);
  return $class;
}
// Get all classes.
...
function edit_subject($subject_id = '') {
    ...
    $wpdb->update($wpdb->prefix . 'ekattor_subject', $data, array('subject_id' => $subject_id));
}

// Delete a subject.
function delete_subject($subject_id = '') {
    global $wpdb;
    $wpdb->delete($wpdb->prefix . 'ekattor_subject', array('subject_id' => $subject_id));
}

// Get information of certain subject.
function get_subject_name_from_id($subject_id = '') {
    global $wpdb;
    $school_subject = $wpdb->prefix . 'ekattor_subject';
    $query_result = $wpdb->get_results("SELECT * FROM $school_subject WHERE subject_id = '$subject_id' ", ARRAY_A);
...
function create_book() {
    ...
    if ($_POST['name'] != "") {
        ...
        $wpdb->insert($wpdb->prefix . 'ekattor_book', $data);
    }
}
...
function get_books() {
    global $wpdb;
    $school_book = $wpdb->prefix . 'ekattor_book';
    $query_result = $wpdb->get_results("SELECT * FROM $school_book ", ARRAY_A);
    return $query_result;
}

// Get information of certain book.
function get_book_info($book_id = '') {
    global $wpdb;
    $school_book = $wpdb->prefix . 'ekattor_book';
    $query_result = $wpdb->get_results("SELECT * FROM $school_book WHERE book_id = '$book_id' ", ARRAY_A);
    return $query_result;
}

// Get books of class of student of logged in parent.
function get_books_of_class_of_student_of_loggedin_parent() {
    global $wpdb;
    $school_book = $wpdb->prefix . 'ekattor_book';
    $school_parent = $wpdb->prefix . 'ekattor_parent';
    $school_student = $wpdb->prefix . 'ekattor_student';
    ...
    $query_result = $wpdb->get_results("SELECT * FROM $school_book WHERE class_id = '$current_user_class_id' ", ARRAY_A);
    return $query_result;
}
...
function edit_book($book_id = '') {
    ...
    $wpdb->update($wpdb->prefix . 'ekattor_book', $data, array('book_id' => $book_id));
}

// Delete a book.
function delete_book($book_id = '') {
    global $wpdb;
    $wpdb->delete($wpdb->prefix . 'ekattor_book', array('book_id' => $book_id));
}
...
function create_transport() {
    ...
    if ($_POST['route_name'] != "") {
        ...
        $data['number_of_vehicle'] = $_POST['number_of_vehicle'];
        $data['description'] = $_POST['description'];
        $data['route_fare'] = $_POST['route_fare'];

        $wpdb->insert($wpdb->prefix . 'ekattor_transport', $data);
    }
}
...
function get_transports() {
    global $wpdb;
    $school_transport = $wpdb->prefix . 'ekattor_transport';
    $query_result = $wpdb->get_results("SELECT * FROM $school_transport ", ARRAY_A);
    return $query_result;
}

// Get information of certain transport.
function get_transport_info($transport_id = '') {
    global $wpdb;
    $school_transport = $wpdb->prefix . 'ekattor_transport';
    $query_result = $wpdb->get_results("SELECT * FROM $school_transport WHERE transport_id = '$transport_id' ", ARRAY_A);
    return $query_result;
}
...
function edit_transport($transport_id = '') {
    ...
    $wpdb->update($wpdb->prefix . 'ekattor_transport', $data, array('transport_id' => $transport_id));
}

// Delete a transport.
function delete_transport($transport_id = '') {
    global $wpdb;
    $wpdb->delete($wpdb->prefix . 'ekattor_transport', array('transport_id' => $transport_id));
}

// Create new dormitory.
function create_dormitory() {
    global $wpdb;
    ...
    if ($_POST['name'] != "") {
        ...
        $wpdb->insert($wpdb->prefix . 'ekattor_dormitory', $data);
    }
}

// Get all dormitories.
function get_dormitorys() {
    global $wpdb;
    $school_dormitory = $wpdb->prefix . 'ekattor_dormitory';
    $query_result = $wpdb->get_results("SELECT * FROM $school_dormitory ", ARRAY_A);
    return $query_result;
}

// Get information of certain dormitory.
function get_dormitory_info($dormitory_id = '') {
    global $wpdb;
    $school_dormitory = $wpdb->prefix . 'ekattor_dormitory';
    $query_result = $wpdb->get_results("SELECT * FROM $school_dormitory WHERE dormitory_id = '$dormitory_id' ", ARRAY_A);
    return $query_result;
}

// Edit dormitory information.
function edit_dormitory($dormitory_id = '') {
    global $wpdb;
    $data['name'] = $_POST['name'];
    $data['number_of_room'] = $_POST['number_of_room'];
    $data['description'] = $_POST['description'];

    $wpdb->update($wpdb->prefix . 'ekattor_dormitory', $data, array('dormitory_id' => $dormitory_id));
}

// Delete a dormitory.
function delete_dormitory($dormitory_id = '') {
    global $wpdb;
    $wpdb->delete($wpdb->prefix . 'ekattor_dormitory', array('dormitory_id' => $dormitory_id));
}
...
function edit_grade($grade_id = '') {
    ...
    $wpdb->update($wpdb->prefix . 'ekattor_grade', $data, array('grade_id' => $grade_id));
}

// Delete a grade.
function delete_grade($grade_id = '') {
    global $wpdb;
    $wpdb->delete($wpdb->prefix . 'ekattor_grade', array('grade_id' => $grade_id));
}

// Create mark entry after verification.
function verify_create_mark_entry($verify_data = '') {
    global $wpdb;
    $school_mark = $wpdb->prefix . 'ekattor_mark';
    $exam_id = $verify_data['exam_id'];
    $class_id = $verify_data['class_id'];
    $subject_id = $verify_data['subject_id'];
    $student_id = $verify_data['student_id'];

    $query_result = $wpdb->get_results("SELECT * FROM $school_mark WHERE student_id = '$student_id' AND exam_id = '$exam_id' AND  class_id = '$class_id' AND  subject_id = '$subject_id' ", ARRAY_A);

    if (empty($query_result)) {
        $wpdb->insert($wpdb->prefix . 'ekattor_mark', $verify_data);
    }
}

// Get mark information according to verify data.
function get_marks($verify_data = '') {
    global $wpdb;
    $school_mark = $wpdb->prefix . 'ekattor_mark';
    $exam_id = $verify_data['exam_id'];
    $class_id = $verify_data['class_id'];
...
// Get highest marks according to verify data.
function get_highest_marks($verify_data = '') {
    global $wpdb;
    $school_mark = $wpdb->prefix . 'ekattor_mark';
    $exam_id = $verify_data['exam_id'];
    $class_id = $verify_data['class_id'];
    ...
    $query_result = $wpdb->get_results("SELECT mark_obtained FROM $school_mark WHERE exam_id = '$exam_id' AND  class_id = '$class_id' AND  subject_id = '$subject_id' ORDER BY mark_obtained DESC LIMIT 1", ARRAY_A);

    return $query_result;
}

// Update mark information.
function update_marks($class_id = '') {
    global $wpdb;
    $class_id              = $class_id;
    $student               = $wpdb->prefix.'ekattor_student';
    $query                 = $wpdb->get_results("SELECT * FROM $student where class_id = '$class_id'" , ARRAY_A);

    foreach ($query as $row):
        $data['mark_obtained'] = $_POST['mark_obtained_'.$row['student_id']];
        $data['comment'] = $_POST['comment_'.$row['student_id']];
        $wpdb->update($wpdb->prefix . 'ekattor_mark', array('mark_obtained'=>$data['mark_obtained'], 'comment'=>$data['comment']), array('mark_id' => $_POST['mark_id_'.$row['student_id']]));
    endforeach;


}

// Create attendance entry after verification.
function verify_create_attendance_entry($verify_data = '') {
    global $wpdb;
    $school_attendance = $wpdb->prefix . 'ekattor_attendance';
    $date = $verify_data['date'];
    $student_id = $verify_data['student_id'];

    $query_result = $wpdb->get_results("SELECT * FROM $school_attendance WHERE student_id = '$student_id' AND date = '$date' ", ARRAY_A);

    if (empty($query_result)) {
        $wpdb->insert($wpdb->prefix . 'ekattor_attendance', $verify_data);
    }
}

// Get attendance information according to verify data.
function get_attendance($verify_data = '') {
    global $wpdb;
    $school_attendance = $wpdb->prefix . 'ekattor_attendance';
    $date = $verify_data['date'];
    $student_id = $verify_data['student_id'];
...
// Get total attendance of current date.
function get_todays_total_attendance() {
    global $wpdb;
    $school_attendance = $wpdb->prefix . 'ekattor_attendance';
    ...
    $query_result = $wpdb->get_results("SELECT * FROM $school_attendance WHERE date = '$date' AND status = 1 ", ARRAY_A);
...
function update_attendance() {
    global $wpdb;
    $data['status'] = $_POST['status'];
    $wpdb->update($wpdb->prefix . 'ekattor_attendance', $data, array('attendance_id' => $_POST['attendance_id']));
}
...
function create_invoice() {
    ...
    $data['status'] = $_POST['status'];
    $data['creation_timestamp'] = strtotime($_POST['creation_timestamp']);

    $wpdb->insert($wpdb->prefix . 'ekattor_invoice', $data);
}

// Get all invoices.
function get_invoices() {
    global $wpdb;
    $school_invoice = $wpdb->prefix . 'ekattor_invoice';
    $query_result = $wpdb->get_results("SELECT * FROM $school_invoice ", ARRAY_A);
    return $query_result;
}

// Get information of certain invoice.
function get_invoice_info($invoice_id = '') {
    global $wpdb;
    $school_invoice = $wpdb->prefix . 'ekattor_invoice';
    $query_result = $wpdb->get_results("SELECT * FROM $school_invoice WHERE invoice_id = '$invoice_id' ", ARRAY_A);
    return $query_result;
}

// Get invoices of logged in student.
function get_invoices_of_loggedin_student() {
    global $wpdb;
    $school_invoice = $wpdb->prefix . 'ekattor_invoice';
    $school_student = $wpdb->prefix . 'ekattor_student';
    global $current_user;
    ...
    $query_result = $wpdb->get_results("SELECT * FROM $school_invoice WHERE student_id = '$current_student_id' ", ARRAY_A);
    return $query_result;
}

// Get invoices of student of logged in parent.
function get_invoices_of_student_of_loggedin_parent() {
    global $wpdb;
    $school_invoice = $wpdb->prefix . 'ekattor_invoice';
    $school_parent = $wpdb->prefix . 'ekattor_parent';
    global $current_user;
    ...
    $query_result = $wpdb->get_results("SELECT * FROM $school_invoice WHERE student_id = '$student_id' ", ARRAY_A);
    return $query_result;
}
...
function edit_invoice($invoice_id = '') {
    ...
    $wpdb->update($wpdb->prefix . 'ekattor_invoice', $data, array('invoice_id' => $invoice_id));
}

// Delete an invoice.
function delete_invoice($invoice_id = '') {
    global $wpdb;
    $wpdb->delete($wpdb->prefix . 'ekattor_invoice', array('invoice_id' => $invoice_id));
}

// Get invoice name from invoice id.
function get_invoice_name_from_id($invoice_id = '') {
    global $wpdb;
    $school_invoice = $wpdb->prefix . 'ekattor_invoice';
    $query_result = $wpdb->get_results("SELECT * FROM $school_invoice WHERE invoice_id = '$invoice_id' ", ARRAY_A);
...
function get_logo_url() {
    $image_url = plugins_url() . '/ekattor/assets/images/logo.png';
    return $image_url;
}
...
function get_documents() {
    global $wpdb;
    $school_document = $wpdb->prefix . 'ekattor_document';
    $query_result = $wpdb->get_results("SELECT * FROM $school_document ORDER BY timestamp DESC ", ARRAY_A);
    return $query_result;
}

// Get information of certain document.
function get_document_info($document_id = '') {
    global $wpdb;
    $school_document = $wpdb->prefix . 'ekattor_document';
    $query_result = $wpdb->get_results("SELECT * FROM $school_document WHERE document_id = '$document_id' ", ARRAY_A);
    return $query_result;
}

// Get documents of logged in student.
function get_documents_of_loggedin_student() {
    global $wpdb;
    $school_document = $wpdb->prefix . 'ekattor_document';
    $school_student = $wpdb->prefix . 'ekattor_student';
    global $current_user;
    ...
    $query_result = $wpdb->get_results("SELECT * FROM $school_document WHERE class_id = '$current_user_class_id' ORDER BY timestamp DESC ", ARRAY_A);
    return $query_result;
}

// Get documents of logged in teacher.
function get_documents_of_loggedin_teacher() {
    global $wpdb;
    $school_document = $wpdb->prefix . 'ekattor_document';
    $school_teacher = $wpdb->prefix . 'ekattor_teacher';
    global $current_user;
    ...
    $query_result = $wpdb->get_results("SELECT * FROM $school_document WHERE teacher_id = '$teacher_id' ORDER BY timestamp DESC ", ARRAY_A);
    return $query_result;
}
...
function edit_document($document_id = '') {
    ...
    $wpdb->update($wpdb->prefix . 'ekattor_document', $data, array('document_id' => $document_id));
}

// Delete a document.
function delete_document($document_id = '') {
    global $wpdb;
    $wpdb->delete($wpdb->prefix . 'ekattor_document', array('document_id' => $document_id));
}


function demo($id='') {
    global $wpdb;
    $wpdb->insert($wpdb->prefix . 'demo', array("dummy"=>$id));
}
